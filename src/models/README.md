...
## Интегрирование языковой модели
В данной реализации проекта использовалась бесплатная версия модели Gigachat. Для получения ключа следует так пройтись по туториалу, который находится по ссылке: https://developers.sber.ru/docs/ru/gigachat/quickstart/ind-create-project. Следующим шагом необходимо создать файл congi.py и добавить туда ключ полученные на первом шаге. Ниже приведен пример
``` python
# название ключа должно быть именно GIGACHAT_AUT_KEY
GIGACHAT_AUT_KEY = '<autr_key>'
```
